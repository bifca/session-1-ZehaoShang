...
if ((($_FILES["file"]["type"] == "image/gif") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/pjpeg") || ($_FILES["file"]["type"] == "image/x-png") || ($_FILES["file"]["type"] == "image/png")) && ($_FILES["file"]["size"] < 20480000) //Size limit by 20Mb
 && in_array($extension, $allowedExts)) {
    if ($_FILES["file"]["error"] > 0) {
        ...
        header("location: carouselCreateForm.php");
        $_SESSION["carouselCreateError"] = "Your upload image file is invalid! Try again.";
    } else {
        ...
        if (file_exists("img/" . $_FILES["file"]["name"])) {
        		header("location: carouselCreateForm.php");
            $_SESSION["carouselCreateError"] =  $_FILES["file"]["name"] . " has uploaded!!!";
        } else {
			...
			$carouselCreate = "INSERT INTO `carouselMedia`(`src`,`type`) VALUE ('" . $imgSrc . "','image')";
	           //If it is successful it will redirect you to the home page.
	        if (mysqli_query($connection, $carouselCreate)) {
	            header("Location: index.php");
	        }
	        //Error Info for Connection
	        else {
	            header("location: carouselCreateForm.php");
	            $_SESSION["carouselCreateError"] = "Sorry, Can't Connect with Sever!";
	        }
        }
